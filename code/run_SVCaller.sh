...
Options:
    -h 				(this menu)
    -a|--bamfile 		(position-sorted bam file)
    -b|--bam_ns 		(name-sorted bam file)
    ...
    -r|--splitter_path		(path to split-reads bam file)
...
while getopts “ha:b:c:d:e:f:g:i:j:k:l:m:n:o:p:q:r:s:t:u:v:w” OPTION
do
...
then
     usage
     echo "Please provide paths to input BAM files as well as SAMTOOLS"
     exit 1
fi
